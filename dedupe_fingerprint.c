...
char *calculate_file_md5(const char *data, int datalen) {
...
int hashcode(char cksum[]) {
	int dec = hextodc(cksum);
	return dec % HASH_SIZE;
}

struct dedupe_footprint *search_dedupe_node(char cksum[]) {
...
int restore_dedupe_fromhash(char *in_file, char *out_file) {
...
int insert_fileobject_inhash(struct dedupe_footprint *obj) {
...
int insert_metadata_node(char *buff, char* filename, char *cksum, int file_offset, int data_len) {
